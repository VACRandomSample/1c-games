...
Процедура УбратьИгровоеПоле()
	СписокЭлементовДляУдаления = Новый Массив;
	Для Каждого ДляУдаления Из Элементы.ИгровоеПоле.ПодчиненныеЭлементы Цикл
		СписокЭлементовДляУдаления.Добавить(ДляУдаления);
	КонецЦикла;

	Для каждого ДляУдаления Из СписокЭлементовДляУдаления Цикл
		ЭтаФорма.Элементы.Удалить(ДляУдаления);
	КонецЦикла;

	ПараметрыПолей.Очистить();
	Параметры.МинВсего = 0;
	Параметры.МинОтмечено = 0;
	ОсталосьМин = 0;
КонецПроцедуры
...
Процедура Подготовка_Поля()
	...
	Параметры.МинВсего = КоличествоМин;
	Параметры.МинОтмечено = 0;
	ОсталосьМин = КоличествоМин;
...
Процедура ЭтоМина(Команда)
	...
	Если ТекущийЭлемент.Заголовок = "Х" Тогда
		Параметры.МинОтмечено = Параметры.МинОтмечено + 1;
		ОсталосьМин = Параметры.МинВсего - Параметры.МинОтмечено;
	КонецЕсли;

	Если Параметры.МинОтмечено = Параметры.МинВсего Тогда
		Предупреждение("Вы выйграли!");
	КонецЕсли;
КонецПроцедуры
